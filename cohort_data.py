...
def sort_by_cohort(filename):
    ...
    for line in f.readlines():
        ...
        if line_list[4] == 'Winter 2016':
            winter_16.append(' '.join(line_list[0:2]))
        elif line_list[4] == 'Spring 2016':
            spring_16.append(' '.join(line_list[0:2]))
        elif line_list[4] == 'Summer 2016':
            summer_16.append(' '.join(line_list[0:2]))
        elif line_list[4] == 'Fall 2015':
            fall_15.append(' '.join(line_list[0:2]))
        elif line_list[4] == 'G':
            ghosts.append(' '.join(line_list[0:2]))
...
def hogwarts_by_house(filename, is_full_name=False):
...
def all_students_tuple_list(filename):
    ...
    for line in f.readlines():
        line_list = line.rstrip().split('|')
        if line_list[2]:
            line_list[0] = ' '.join(line_list[0:2])
            line_list.pop(1)
            student_list.append(tuple(line_list))

    # student_list = [tuple(i.rstrip().split('|')) for i in f if i.rstrip().split('|')[2]]

    return student_list


def find_cohort_by_student_name(student_list, is_raw_data=False):
    ...
    for student in student_list:
        if student[0].lower() == user_input.lower():
            if is_raw_data:
                return student
            else:
                return '{} was in the {} cohort.'.format(student[0], student[3])
...
def find_house_members_by_student_name(student_list):
    """TODO: Prompt user for a student. Display everyone in their house and cohort.
     ...
     """
    student = find_cohort_by_student_name(student_list, True)
    student_name = student[0]
    student_house = student[1]
    student_cohort = student[3]
    print '{} was in house {} in the {} cohort.'.format(
        student_name, student_house, student_cohort)
    print 'The following students are also in their house:'

    for student_data in student_list:
        if student_data[1] == student_house \
          and student_data[3] == student_cohort \
          and student_data[0] != student_name:
            print student_data[0]

    # get_houses = hogwarts_by_house(FILE_NAME, True)
    # print get_houses
...
all_students_data = all_students_tuple_list(FILE_NAME)
